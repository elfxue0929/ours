...
from multiview_stem import MultiViewStem
from multiview_fusion import MultiViewFusion, AdaptiveMultiViewFusion
from view_aware_attention import ViewAwareAttention
...
class SABlock(nn.Module):
    def __init__(self, dim, num_heads, sparse_size=0, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, use_view_aware=False, view_alpha=1.0, sparse_topk=None):
        super().__init__()
        self.pos_embed = nn.Conv2d(dim, dim, 3, padding=1, groups=dim)
        self.norm1 = norm_layer(dim)
        # Use ViewAwareAttention if enabled, otherwise standard Attention
        if use_view_aware:
...
class SparseViT(nn.Module):
    def __init__(self, layers=[5, 8, 20, 7], img_size=224, in_chans=3, s_blocks3=[ 8, 4, 2, 1], s_blocks4=[2, 1], embed_dim=[64, 128, 320, 512],
                 ...
                 use_view_aware=False, view_alpha=1.0, sparse_topk=None, fusion_type='concat_proj'
                 ):
        super().__init__()
        ...
        # Multi-View Stem
        self.mv_stem = MultiViewStem(
            in_ch=in_chans,
            c_rgb=embed_dim[0],
            c_noise=embed_dim[0] // 2,
            c_freq=embed_dim[0] // 2
        )
        
        # Multi-View Fusion
        if fusion_type == 'adaptive':
            self.mv_fusion = AdaptiveMultiViewFusion(
                c_rgb=embed_dim[0],
                c_noise=embed_dim[0] // 2,
                c_freq=embed_dim[0] // 2,
                c_out=embed_dim[0]
            ...
            self.mv_fusion = MultiViewFusion(
                c_rgb=embed_dim[0],
                c_noise=embed_dim[0] // 2,
                c_freq=embed_dim[0] // 2,
                c_out=embed_dim[0],
                fusion_type=fusion_type  # Options: 'concat_proj', 'weighted_sum', 'attention'
            )
        ...
        for i in range(layers[2]):
            block =  SABlock(
                            ...
                            use_view_aware=use_view_aware, view_alpha=view_alpha, sparse_topk=sparse_topk)
    ...
    def _uniformer_init_weights(self):
        if self.pretrained_path != None:
            # PyTorch 2.6+ compatibility: handle weights_only parameter
            try:
    ...
    def forward_features(self, x, return_gate: bool = False):
        outputs = {}
        # Multi-View Stem extraction
        F_rgb, F_noise, F_freq = self.mv_stem(x)
        # Multi-View Fusion (Step 3)
        gate = None
        if return_gate:
            fusion_out = self.mv_fusion(F_rgb, F_noise, F_freq, return_gate=True)
            if isinstance(fusion_out, (tuple, list)) and len(fusion_out) == 2:
                x, gate = fusion_out
            else:
                x = fusion_out
        else:
            x = self.mv_fusion(F_rgb, F_noise, F_freq)
        x = self.pos_drop(x)
    ...
    def forward(self, x, return_gate: bool = False):
        outputs = self.forward_features(x, return_gate=return_gate)
        return outputs
